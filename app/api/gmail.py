...
def _process_new_threads(service: Any, history_id: str) -> None:
    ...
    try:
        ...
        if last_history_id is None:
            ...
            logger.info(
                "No prior historyId in DB; bootstrapping from %s", last_history_id
            )
        ...
        try:
            history_result = fetch_history_since(service, last_history_id)
        except HttpError:
            logger.exception(
                "History API fetch failed; not advancing stored historyId. "
                "last_history_id=%s notification_history_id=%s",
                last_history_id,
                history_id,
            )
            return
        ...
        logger.info(
            "Found %d new thread(s) to process.", len(history_result.thread_ids)
        )

        label_name_map = get_label_name_map(service)

        for thread_id in history_result.thread_ids:
            try:
                thread = (
                    service.users()
                    .threads()
        ...
        # Persist only after a successful History API fetch.
        newest_history_id = history_result.latest_history_id or history_id
        crud_app_state.set_value(db, _GMAIL_HISTORY_ID_KEY, newest_history_id)
        logger.info("Updated stored historyId to %s", newest_history_id)
    finally:
        db.close()
...
def gmail_webhook(
    ...
    try:
        raw_data = payload.message.data.strip()
        padded_data = raw_data + ("=" * (-len(raw_data) % 4))
        decoded = base64.b64decode(padded_data, validate=True).decode("utf-8")
    ...
    background_tasks.add_task(_process_new_threads, service, history_id)
